...
class BlogCreationForm(ModelForm):
    ...
    def save(self, user, commit=True):
        ...
        post.user = user  
        post.thumbnail.save(name, post.image)
        if commit: 
            post.save()
...
class IngredientsForm(ModelForm):
    ...
    metric = forms.ChoiceField(widget=forms.Select(attrs={'class': 'form-control', 'required': True}), choices={('Tbsp', 'Tbsp'), ('ml', 'ml'), ('gr', 'gr'), ('kg', 'kg'), ('portion', 'portion'),('cup', 'cup')}, label='Units')
